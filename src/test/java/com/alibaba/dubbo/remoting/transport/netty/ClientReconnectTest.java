...
public class ClientReconnectTest {
    ...
    public void testReconnectWarnLog() throws RemotingException, InterruptedException {
        ...
        String url = "exchange://127.0.0.2:" + port + "/client.reconnect.test?client=netty5&check=false&"
    ...
    public void testReconnectErrorLog() throws RemotingException, InterruptedException {
        int port = NetUtils.getAvailablePort();
        DubboAppender.doStart();
        String url = "exchange://127.0.0.3:" + port + "/client.reconnect.test?client=netty5&check=false&"
                + Constants.RECONNECT_KEY + "=" + 1 + // 1ms reconnect,保证有足够频率的重连
    ...
    public Client startClient(int port, int reconnectPeriod) throws RemotingException {
        final String url = "exchange://127.0.0.1:" + port + "/client.reconnect.test?client=netty5&check=false&" + Constants.RECONNECT_KEY + "=" + reconnectPeriod;
        return Exchangers.connect(url);
    }

    public Server startServer(int port) throws RemotingException {
        final String url = "exchange://127.0.0.1:" + port + "/client.reconnect.test?server=netty5";
